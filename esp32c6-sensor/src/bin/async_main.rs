...
//pub const L2CAP_MTU: usize = 1037;
...
async fn main(_spawner: Spawner) {
    ...
    //let maybe_init = esp_radio::init(timer_group1.timer0, esp_hal::rng::Rng::new(peripherals.RNG));
    ...
    // For a random static address, the two most significant bits must be '11'
    // Modify the last byte (most significant) to meet BLE requirements
    let mut ble_mac = mac;
    ble_mac[5] |= 0xC0;  // Set top two bits to 11
    let mac_address: Address = Address::random(ble_mac);

    let address: Address = Address::random([0xff, 0x8f, 0x1a, 0x05, 0xe4, 0xff]);
    info!("Our address = {:?}, mac {mac_address:?}", address);

    let mut resources: HostResources<DefaultPacketPool, 0, 0, 27> = HostResources::new();
    let stack = trouble_host::new(controller, &mut resources).set_random_address(mac_address);
    let Host {
        mut peripheral,
        runner,
        ..
